...
tree = BinaryTree()
assert minimum_depth(tree) == 0

tree = BinaryTree()
tree.insert(10)
assert minimum_depth(tree) == 1
...
for x in [4, 2, 6, 1, 3, 5, 7]:
    tree.insert(x)
assert minimum_depth(tree) == 3

tree = BinaryTree()
for x in [5, 4, 3, 2]:
    tree.insert(x)
assert minimum_depth(tree) == 4
...
root.right = BinaryNode(4, None, None)
tree.head = root
assert minimum_depth(tree) == 2
